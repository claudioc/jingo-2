...
test('checkDocumentRoot fails when document root is empty', async t => {
  ...
  const error = await t.throwsAsync(validators.checkDocumentRoot(cfg, undefined));
...
test('checkDocumentRoot fails if the document root is not writable', async t => {
  const cfg = await config();
  const value = '/';
  const error = await t.throwsAsync(validators.checkDocumentRoot(cfg, value));
...
test('checkDocumentRoot success on a readable directory', async t => {
  ...
  await t.notThrowsAsync(validators.checkDocumentRoot(cfg, value));
});
